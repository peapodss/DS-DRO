...
static BOOL RegQueryValueString(HKEY kKey, char *lpValueName, char **pszValue) {
  *pszValue = NULL;
  DWORD dwType = 0;
  DWORD dwDataSize = 0;
  ...
  *pszValue[0] = 0;
  ...
  nError = RegQueryValueEx(kKey, lpValueName, NULL, &dwType, (LPBYTE)*pszValue, &dwReturnedSize);
...
static bool IsNumeric(char *pszString, bool bIgnoreColon) {
  ...
  while (bMoreItems) {
    ...
    if (bMoreItems) {
      ...
      if(bAdded) {
        ...
        DWORD dwType = 0;
        if(SetupDiGetDeviceRegistryProperty(h_devinfo, &devInfo, SPDRP_DEVICEDESC, &dwType, (PBYTE)szFriendlyName, dwSize, &dwSize) && (dwType == REG_SZ)) {
